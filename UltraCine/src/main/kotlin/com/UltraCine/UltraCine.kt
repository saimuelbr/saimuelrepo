...
class UltraCine : MainAPI() {
  ...
  override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
    ...
    val home =
        app.get(url).document.select("div.aa-cn div#movies-a ul.post-lst li").mapNotNull {
          it.toSearchResult()
        }
  ...
  private fun Element.toSearchResult(): SearchResponse? {
    ...
    val ratingText = selectFirst("div.entry-meta span.vote")?.text()?.replace("TMDB", "")?.trim()
  ...
  override suspend fun load(url: String): LoadResponse? {
    ...
    val title =
        document.selectFirst("aside.fg1 header.entry-header h1.entry-title")?.text() ?: return null
    ...
    val iframeUrl = document.selectFirst("iframe[src*='painel-aso.top']")?.attr("src")
  ...
  override suspend fun loadLinks(
    ...
    val targetUrl =
        when {
          data.matches(Regex("\\d+")) -> "https://painel-aso.top/episodio/$data"

          data.contains("ultracine.org") && data.split("/").last().matches(Regex("\\d+")) ->
              "https://painel-aso.top/episodio/${data.split("/").last()}"

          data.contains("ultracine.org") ->
              app.get(data)
                  .document
                  .selectFirst("iframe[src*='painel-aso.top']")
                  ?.attr("src") ?: data
    ...
    links.forEach { link ->
      when {
        ...
        else -> loadExtractor(link, targetUrl, subtitleCallback, callback)
      }
    }
  ...
  private fun extractEmbedLinks(doc: Document): List<String> {
    ...
    val iframes =
        doc.select("div#player iframe, div.play-overlay iframe").mapNotNull { it.attr("src") }
    return (buttons + iframes).distinct().filter { it.isNotBlank() }
  }

  private fun Document.selectActors(): List<Pair<Actor, ActorRole?>>? {
    val actors = select("ul.cast-lst a").map { Actor(it.text(), it.attr("href")) to null }
  ...
  private fun Document.extractText(selector: String): String? = selectFirst(selector)?.text()
